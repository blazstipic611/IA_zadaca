...
to setup
  ...
  set target-coordinates-list [] ; Clear the target-coordinates-list
end
...
to walk1
  if ([pcolor] of patch-here = orange) [
    if not (member? "Orange" samples1 ) [
      ...
      show samples1
      let target-patch patch-ahead 1 ; Get the patch in front of the agent
      let target-coordinates list [pxcor] of target-patch [pycor] of target-patch ; Get the coordinates of the target patch
      set target-coordinates-list lput target-coordinates target-coordinates-list
      set counter counter + 1 ; Increment the counter variable
    ]
  ]
...
to walk2
  if not empty? target-coordinates-list [
    ...
    let heading-patch patch item 0 target-proba item 1 target-proba
    face heading-patch

    while [(pxcor != item 1 target-proba) or (pycor != item 1 target-proba)] [
