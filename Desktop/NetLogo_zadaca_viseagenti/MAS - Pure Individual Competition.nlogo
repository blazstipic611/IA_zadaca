turtles-own [ samples1 samples2 samples3 samples4 ]
...
to go
  ...
  [ stop ]
  if ([length samples1] of turtle 0 = 3) or ([length samples2] of turtle 1 = 3) or ([length samples3] of turtle 2 = 3) or ([length samples4] of turtle 3 = 3)
  [
    ;; odredjuje se "pobjednik" natjecanja
    ifelse ([length samples1] of turtle 0 = 3)
    [ ask turtle 0 [win]
      ask turtle 1 [lose]
       ]
    [ ifelse ([length samples2] of turtle 1 = 3)
      [ ask turtle 1 [win]
        ask turtle 0 [lose]
         ]
      [ ifelse ([length samples3] of turtle 2 = 3)
        [ ask turtle 0 [win]
          ask turtle 1 [lose]
           ]
        [ ifelse ([length samples4] of turtle 3 = 3)
          [ ask turtle 1 [win]
            ask turtle 0 [lose]
             ]
          [ stop ] ] ] ]
...
to walk3  ;; funkcija treceg rovera
  if ([pcolor] of patch-here = orange)
  [
    if not (member? "Orange" samples3 )
    [
      set samples3 lput "Orange" samples3
      show samples3
    ]
  ]
  if ([pcolor] of patch-here = red)
  [
    if not (member? "Red" samples3 )
    [
      set samples3 lput "Red" samples3
      show samples3
    ]
  ]
  if ([pcolor] of patch-here = blue)
  [
    if not (member? "Blue" samples3 )
    [
      set samples3 lput "Blue" samples3
      show samples3
    ]
  ]
...
to walk4  ;; funkcija cetvrtog rovera
  if ([pcolor] of patch-here = orange)
  [
    if not (member? "Orange" samples4 )
    [
      set samples4 lput "Orange" samples4
      show samples4
    ]
  ]
  if ([pcolor] of patch-here = red)
  [
    if not (member? "Red" samples4 )
    [
      set samples4 lput "Red" samples4
      show samples4
    ]
  ]
  if ([pcolor] of patch-here = blue)
  [
    if not (member? "Blue" samples4 )
    [
      set samples4 lput "Blue" samples4
      show samples4
    ]
  ]
  ifelse (ycor = 6) and (heading = 0)
