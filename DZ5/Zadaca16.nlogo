globals [ pomocna pomocna1 ]
breed [ cat cats ]
breed [ catkid catskids ]
breed [ mouse mouses ]
...
to go
  if not any? turtles with [breed = mouse][
    stop
  ]
  parent-cat
  parent1-cat
...
to parent-cat
   ...
   if any? turtles with [breed = mouse][
    ask turtle 0[
      set pomocna min-one-of patches with [any? turtles-here with [breed = mouse]][distance myself]
      if pomocna != nobody[
        face pomocna
        fd 1
        if any? mouse-here [
...
to parent1-cat
   ...
   if any? turtles with [breed = mouse][
    ask turtle 2[
      set pomocna1 min-one-of patches with [any? turtles-here with [breed = mouse]][distance myself]
      if pomocna1 != nobody[
        face pomocna1
        fd 1
        if any? mouse-here [
...
to dijete1
  ask turtle 3[
    while [patch-here != pomocna1][
      face pomocna1
      fd 1
    ]
    if patch-here = pomocna1[
      ask mouse-here[
            die
          ]
      while[patch-here != patch 8 -6][
